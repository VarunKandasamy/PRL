...
//PORTS --------------<---0---|---1---|---2---|---3---|---4---|---5--->
float grey_value[6] = {1500   ,1500   ,1500   ,1500   ,3000,3000};
float error_mod[6] =  {0.5    ,0.5    ,0.3    ,0.5    ,0.5    ,0.7};
//PORTS --------------/---0---/---1---/---2---/---3---/---4---/---5---/
...
int in_range(float current, float desired, float spread){
    if(current > desired - spread && current < desired + spread){
        return 1;
    }else{
        return 0;
    }
...
void spin_motor(int ticks, int speed, int port){
    int start = gmpc(port);
    while(abs(start - gmpc(port)) < ticks){
        mav(port, speed);
    }
    mav(port,0);
    msleep(20);
}
void spin_motor_smooth(int ticks, float speed, int port){
   	float save = accel_distance;
    accel_distance = ticks/4.5;
    int start = gmpc(port);
    while(abs(start - gmpc(port)) < ticks){
        float speed_mult = calculate_speed_ramp(ticks, abs(start-gmpc(port)));
        mav(port, speed*speed_mult);
    }
    mav(port,0);
    msleep(20);
    accel_distance = save;
...
void d_line_follow(float distance, float speed, int port, char side){
    ...
    double start = seconds();
    while(y < distance){
       	float error = grey_value[port] - analog(port);
        viprint(analog(port));
        float speed_mod = error*error_mod[port];
       	if(side == 'l'){
            speed_mod *= -1;
        }
...
void line_follow_till_an(float distance, float speed, int port,int s_port, char side){
    clear_wheels();
    long double x = 0;
    long double y = 0;
    long double theta = pi/2;
    ...
    float target_speed = (tps + 2)/1.08;
    double start = seconds();
    while(analog(s_port) < grey_value[port]){
       	float error = grey_value[port] - analog(port);
        viprint(analog(port));
        float speed_mod = error*error_mod[port];
       	if(side == 'l'){
...
void r_line_follow(float distance, float speed, int port, char side){
    ...
    while(fabs(dist_travelled) < distance){
        float speed_adj = (analog(port)-grey_value[port]) * error_mod[port];
        if(side == 'r'){
            speed_adj = -speed_adj;
        }
...
void create_square_up(int speed){
    ...
    float spd;
    if((chain_size == chain || chain < 1)){
		spd = 0;
    }else{
        spd = speed;
    }
    double start_time = seconds();
    while(get_create_lcliff_amt() > 2500 || get_create_rcliff_amt() > 2500){
     
        if(get_create_lcliff_amt() > 2500){
        ...
        step_servos(seconds()-start_time);
		start_time = seconds();
        if(abs(spd) < abs(speed)){
            spd += 10 * speed/fabs(speed);
        }
    }
    create_drive_direct(0,0);
    msleep(10);
